...
def _get_db_url(connector: str) -> str:
    ...
    return f'user={connection.login} password={connection.password} host={connection.host} port={connection.port} dbname={connection.schema}'
...
def get_data_from_signal_table (connector:str, filter_:str) -> pd.DataFrame:
    query=f"""
        Select time, position, strategy_type, ticker FROM signal {filter_}
    """
    with psycopg2.connect(dsn=_get_db_url(connector)) as conn:
        data=pd.read_sql(query,conn)
    return data